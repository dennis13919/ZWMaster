...
import datetime
import traceback
# import csv # Removed, as import/export CSV is gone
import time
# import json # Not used directly, but often for client_storage. Keep for now.
import os
from logic import ZiweiCalculator
# from database import ZiweiDatabase # Removed
...
本系統提供專業的南北派紫微斗數排盤功能，包含動態大限流年切換以及完整的資料庫管理系統。
設定選單並提供多種繪圖自定義參數，以完美符合各比例行動裝置。
V5.9 修正北派星曜顯示。
** 三方四正 或 北派自化線 出錯處理: 調整瀏覽器大小，會自動重抓位置。
...
Windows及Android版本下載 https://bit.ly/ZWMlatest
"""
...
UI_CFG = {
    "appbar_title": 16,
    "appbar_height": 45,
    # "db_list_font": 12, # Removed, no DB list
    # "db_list_height": 45, # Removed, no DB list
    "setting_font": 13,
    "setting_title": 14,
    "input_label": 12,
    "btn_font": 12,
...
def main(page: ft.Page):
    ...
    # db = ZiweiDatabase() # Removed
    calculator = ZiweiCalculator()

    # def get_categories(): # Removed, no DB categories
    #     return [
    ...
    app_state = {
        ...
        # "edit_uid": None, # Removed, no DB editing
    }
    ...
    def add_text_to_canvas(e):
        ...
        # 動態大小：基礎 15 + 筆觸粗細 * 2 + 全域偏移
        dynamic_size = 15 + (app_state["stroke_width"] * 2) + app_state["font_offset"]
    ...
    def page_resize(e):
        ...
        if app_state["full_data"]:
            draw_chart()
            # 這裡多加一個 draw_overlay 確保線條在旋轉後立刻跟上
            # draw_overlay()
    ...
    def do_calculate_logic(birth_data):
        ...
        res['is_lunar'] = birth_data.get("is_lunar", False)  # 補上這行
        res['minute'] = birth_data.get("minute", 0)  # 補上這行
        res['note'] = birth_data.get("note", "")  # 補上這行
        # res['category'] = birth_data.get("category", "")  # Removed, no DB category
    ...
    def on_dialog_submit(e):
        ...
        try:
            birth_data = {
                ...
                # "category": input_category.value, # Removed, no DB category
                "note": input_note.value
            }

            # if app_state.get("edit_uid") is not None: # Removed, no DB editing
            #     db.update_user(app_state["edit_uid"], birth_data)
            #     show_snack("已更新資料", "green700")
            #     refresh_db_list()
            # else:
            # 新增：按下排盤或儲存時，記憶當前選取的分類 # Removed, no DB category
            # settings.set("last_input_category", input_category.value) # Removed, no DB category
            # if save_db_cb.value: # Removed, no DB saving
    ...
    def draw_chart():
        ...
        def get_p(idx):
            ...
            # 關鍵修改：明確指定未選中時背景為透明
            #    if app_state["selected_palace_idx"] == idx:
            #        cell.bgcolor = ft.colors.with_opacity(0.3, "yellow50")  # 選中時帶點透明黃
            #    else:
            #        cell.bgcolor = ft.colors.TRANSPARENT  # 未選中時完全透明
    ...
    # def on_export_result(e: ft.FilePickerResultEvent): # Removed, no DB import/export
    #     if e.path:
    #         try:
    #             # --- 修改：加入時間戳記，避免檔名重複與覆寫失敗 ---
    ...
    #                 writer = csv.writer(f)
    #                 writer.writerow(["姓名", "性別", "生辰", "備註"])
    #                 for u in users:
    #                     birth_str = f"{u['year']}{u['month']:02d}{u['day']:02d}{u['hour']:02d}{u['minute']:02d}"
    #                     writer.writerow([u['name'], u['gender'], birth_str, u['note']])
    #             show_snack(f"匯出成功: {filename}", "green700")  # 提示改顯示檔名
    #         except Exception as ex:
    #             show_snack(f"匯出失敗: {str(ex)}", "red800")

    # def on_import_result(e: ft.FilePickerResultEvent): # Removed, no DB import/export
    ...
    #                                 "is_lunar": False, "category": target_cat, "note": note
    #                             }
    #                             db.add_user(data)
    #                             count += 1
    ...
    # export_picker = ft.FilePicker(on_result=on_export_result) # Removed, no DB import/export
    # import_picker = ft.FilePicker(on_result=on_import_result) # Removed, no DB import/export
    # page.overlay.extend([export_picker, import_picker]) # Removed, no DB import/export

    # db_state = {"category": "全部", "keyword": "", "selected_ids": set(), "is_select_mode": False, "auto_anon": False } # Removed, no DB state
    # db_list_view = ft.ListView(expand=True, spacing=4, padding=4) # Removed, no DB list
    # db_appbar = ft.AppBar(toolbar_height=UI_CFG["appbar_height"]) # Removed, no DB appbar

    # def refresh_db_list(): # Removed, no DB list
    ...
    # def exit_select_mode(e=None): # Removed, no DB list
    #     db_state["is_select_mode"] = False
    #     db_state["selected_ids"].clear()
    #     refresh_db_list()

    # def delete_selected(e): # Removed, no DB list
    ...
    # def move_selected(e): # Removed, no DB list
    #     move_dd = ft.Dropdown(
    ...
    # def edit_selected(e): # Removed, no DB list
    ...
    # def update_db_appbar(): # Removed, no DB appbar
    #     if db_state["is_select_mode"]:
    ...
    # def open_db_view(): # Removed, no DB view
    ...
    def open_settings_view():
        def save_settings(e):
            ...
            # settings.set("cat_1", cat1_tf.value) # Removed, no DB categories
            # settings.set("cat_2", cat2_tf.value) # Removed, no DB categories
            # settings.set("cat_3", cat3_tf.value) # Removed, no DB categories
            # settings.set("cat_4", cat4_tf.value) # Removed, no DB categories
            # settings.set("cat_5", cat5_tf.value) # Removed, no DB categories
            ...
            if app_state["full_data"]:
                ...
                do_calculate_logic(app_state["birth_info"])
            # input_category.options = [ft.dropdown.Option(c) for c in get_categories()] # Removed, no DB categories
        ...
        # cats = get_categories() # Removed, no DB categories
        # cat1_tf = ft.TextField(label="分類 1", value=cats[0], expand=1, **tf_style) # Removed, no DB categories
        # cat2_tf = ft.TextField(label="分類 2", value=cats[1], expand=1, **tf_style) # Removed, no DB categories
        # cat3_tf = ft.TextField(label="分類 3", value=cats[2], expand=1, **tf_style) # Removed, no DB categories
        # cat4_tf = ft.TextField(label="分類 4", value=cats[3], expand=1, **tf_style) # Removed, no DB categories
        # cat5_tf = ft.TextField(label="分類 5", value=cats[4], expand=1, **tf_style) # Removed, no DB categories
        ...
        star_wrap_offset_tf = ft.TextField(label="星曜換行偏移 (+/-)", value=str(settings.get("star_wrap_offset", 0)),
                                           keyboard_type="number", expand=1, **tf_style)
        # 新增：星曜間距偏移輸入框
        star_col_width_offset_tf = ft.TextField(label="星曜左右間距偏移 (+/-)",
                                                value=str(settings.get("star_col_width_offset", 0)),
                                                keyboard_type="number", expand=1, **tf_style)

        minimal_stars_cb = ft.Checkbox(
            label="北派星曜極簡化",
            value=settings.get("minimal_stars", False)
        )
        ...
        return ft.View(
            "/settings", [
                ...
                ft.ListView([
                    ...
                    # ft.Text("資料庫群組命名 (限4字)", weight="bold", color="blue700", size=UI_CFG["setting_title"]), # Removed, no DB categories
                    # ft.Row([cat1_tf, cat2_tf]), # Removed, no DB categories
                    # ft.Row([cat3_tf, cat4_tf]), # Removed, no DB categories
                    # ft.Row([cat5_tf, ft.Container(expand=1)]), ft.Divider(height=10), # Removed, no DB categories
                    ...
                    ft.TextButton(
                        ...
                        on_click=lambda _: [
                            ...
                            show_snack("已重置，請旋轉螢幕重新偵測", "blue700")
                        ]
                    )
    ...
    # def load_user(u, auto_calc=True): # Removed, no DB loading
    #     try:
    ...
    # last_input_cat = settings.get("last_input_category") or get_categories()[0] # Removed, no DB categories
    # input_category = ft.Dropdown(label="分類", expand=2, options=[ft.dropdown.Option(c) for c in get_categories()], # Removed, no DB categories
    #                              value=get_categories()[0], **tf_style) # Removed, no DB categories
    input_note = ft.TextField(label="備註", **tf_style)
    # save_db_cb = ft.Checkbox(label="儲存至資料庫", value=False) # Removed, no DB saving
    ...
    dialog = ft.AlertDialog(
        ...
        content=ft.Column([
            ...
            ft.Row([input_hour, input_minute, ], spacing=5),  # input_category removed
            input_note,
            ft.Column([
                # ft.Row([save_db_cb], alignment="start"), # Removed, no DB saving
    ...
    def open_dialog(e):
        # app_state["edit_uid"] = None # Removed, no DB editing
        dialog_title.value = "輸入生辰"
        submit_btn.text = "排盤"
        # save_db_cb.visible = True # Removed, no DB saving
        hide_name_cb.visible = True
        input_name.value = ""  # Clear name for new input
        input_note.value = ""  # Clear note for new input
        page.open(dialog)
    ...
    def route_change(route):
        page.views.clear()
        # if page.route == "/db": # Removed, no DB view
        #     page.views.append(open_db_view()) # Removed, no DB view
        if page.route == "/settings":
            ...
            if app_state["drawing_mode"]:
                ...
                current_appbar = ft.AppBar(
                    title=ft.Row([
                        ft.Container(content=ft.Image(src="ZWMaster.png", width=28, height=28), border_radius=8,
                                     clip_behavior=ft.ClipBehavior.ANTI_ALIAS),
                        ft.Text("ZWMaster Web", weight="bold", size=UI_CFG["appbar_title"])
                    ], spacing=8),
                    toolbar_height=UI_CFG["appbar_height"], bgcolor="surfaceVariant",
                    # leading=ft.IconButton(ft.Icons.STORAGE, tooltip="資料庫", on_click=lambda e: page.go("/db")), # Removed, no DB view
                    actions=[
                        # 在這裡增加進入繪圖模式的按鈕
                        ft.IconButton(ft.Icons.DRAW, tooltip="進入繪圖模式",
                                      on_click=lambda _: toggle_drawing_mode(True)),
                ...
                current_bottombar = ft.BottomAppBar(
                    ...
                    content=ft.Row([
                        ft.IconButton(ft.Icons.ADD_CIRCLE_OUTLINE, tooltip="輸入排盤", icon_color="blue700",
                                      icon_size=28, on_click=open_dialog),
                        school_toggle_row,
                        ft.IconButton(ft.Icons.CALENDAR_MONTH,
                                      icon_color="blue700" if hide_birth_cb.value else "grey500",
            ...
            page.views.append(ft.View(
                ...
                controls=[chart_container,
                          ft.Container(ft.Column([daxian_row, liunian_row], spacing=0), padding=5, bgcolor="white")],
                padding=0
            ))
        page.update()
    ...
    input_note.value = ""  # Ensure note is cleared on startup
